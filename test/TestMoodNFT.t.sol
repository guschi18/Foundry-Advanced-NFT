...
contract TestMoodNFT is Test {
    ...
    function testFlipMood() public {
        ...
        // After flip, URI should be different
        string memory sadUri = moodNft.tokenURI(0);
        
        // Assert that the URIs are different after flipping
    ...
    function testMoodStartsHappy() public {
        ...
        assertTrue(bytes(initialUri).length > 0);
    }
    ...
    function testCanFlipMoodMultipleTimes() public {
        ...
        assert(keccak256(abi.encodePacked(initialUri)) == keccak256(abi.encodePacked(secondFlipUri)));
        assert(keccak256(abi.encodePacked(initialUri)) != keccak256(abi.encodePacked(firstFlipUri)));
    }
}
